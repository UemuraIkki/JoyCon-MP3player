...
class JoyCon:
    ...
    def set_rumble(self, enable: bool):
        if enable:
            # 振動をオンにするバイト列（例: 低周波の弱い振動）
            # ※左モーター4バイト + 右モーター4バイトの計8バイト
            active_rumble = b'\x80\x20\x62\x14\x80\x20\x62\x14'
            self._RUMBLE_DATA = active_rumble
        else:
            # 振動をオフにする（デフォルトのニュートラル状態）
            neutral_rumble = b'\x00\x01\x40\x40\x00\x01\x40\x40'
            self._RUMBLE_DATA = neutral_rumble
            
        # コマンド0x10は「振動データのみを送信する」という命令
        # サブコマンドと引数は不要なので空のバイト列を渡す
        self._write_output_report(b'\x10', b'', self._RUMBLE_DATA)
